...
public class Election {
    ...
    public List<Candidate> getAllCandidates() {
        ...
        return candidates; // replace by a proper outcome
    }
    ...
    public Map<Constituency, Integer> numberOfRegistrationsByConstituency(Party party) {
        ...
        Map<Constituency, Integer> constituencyIntegerMap = new HashMap<>();

        constituencies.forEach(constituency -> {
            constituencyIntegerMap.put(constituency, constituency.getCandidates(party).size());
        });
        return constituencyIntegerMap; // replace by a proper outcome
    }
    ...
    public Set<Candidate> getCandidatesWithDuplicateNames() {
        ...
        Set<Candidate> nameCountCandidate = new HashSet<>();

        getAllCandidates().forEach(candidate -> {
            nameCount.put(candidate.getFullName(), nameCount.getOrDefault(candidate.getFullName(), 0) + 1);
        });

        nameCount.forEach((s, integer) -> {
            if (integer > 1) {
                getAllCandidates().forEach(c -> {
                    if (c.getFullName().equals(s)) {
                        nameCountCandidate.add(c);
                    }
                });
            }
        });
...
//                getAllCandidates().stream().mapMulti(((candidate, consumer) -> candidate.getFullName()))
        return nameCountCandidate; // replace by a proper outcome
    }
    ...
    public Collection<PollingStation> getPollingStationsByZipCodeRange(String firstZipCode, String lastZipCode) {
        Collection<PollingStation> pollingStations = new HashSet<>();
        constituencies.forEach(constituency -> {
    ...
    public Map<Party, Integer> getVotesByParty() {
        ...
        constituencies.forEach(constituency -> {
            constituency.getVotesByParty().forEach(((party, integer) -> {
                votesByParty.put(party, votesByParty.getOrDefault(party, 0) + integer);
            }));
        });
    ...
    public Map<Party, Integer> getVotesByPartyAcrossPollingStations(Collection<PollingStation> pollingStations) {
        ...
        Map<Party, Integer> votesPollingStations = new HashMap<>();

        pollingStations.forEach(pollingStation -> {
            pollingStation.getVotesByParty().forEach((party, integer) -> {
                votesPollingStations.put(party, votesPollingStations.getOrDefault(party, 0) + integer);
            });
        });

        return votesPollingStations; // replace by a proper outcome
    }
    ...
    public static List<Map.Entry<Party, Double>> sortedElectionResultsByPartyPercentage(int tops, Map<Party, Integer> votesCounts) {
        double totVotes = votesCounts.values().stream().mapToDouble(v -> v).sum();

        Map<Party, Double> votesPercentage = votesCounts.entrySet().stream().sorted((obj1, obj2) -> obj2.getValue().compareTo(obj1.getValue()))
                .limit(tops).collect(Collectors.toMap(Map.Entry::getKey, a -> (100 * a.getValue()) / totVotes));

        return votesPercentage.entrySet().stream().sorted((obj1, obj2) -> obj2.getValue().compareTo(obj1.getValue())).limit(tops).collect(Collectors.toList());
    ...
    public PollingStation findMostRepresentativePollingStation() {
        Map<PollingStation, Double> pollingStationMap = new HashMap<>();
        Map<Party, Integer> votesByParty = getVotesByParty();

        constituencies.stream().map(Constituency::getPollingStations).flatMap(Collection::stream)
                .forEach(pollingStation -> pollingStationMap.put(pollingStation, euclidianVotesDistributionDeviation(votesByParty, pollingStation.getVotesByParty())));

        return Collections.min(pollingStationMap.entrySet(), Map.Entry.comparingByValue()).getKey();
    }
    ...
    public String prepareSummary(int partyId) {
        ...
        System.out.println("Total number of candidates = " + party.getCandidates().size());
        // TODO report the list with all candidates in the given party
        System.out.println("Candidates: " + party.getCandidates());
        // TODO report total number of registrations for the given party
        AtomicInteger totalRegistrations = new AtomicInteger();
        constituencies.forEach(constituency -> totalRegistrations.addAndGet(constituency.getCandidates(party).size()));
        System.out.println("Total number of registrations = " + totalRegistrations);
        // TODO report the map of number of registrations by constituency for the given party
        System.out.print("Number Of registrations per constituency:");
        constituencies.forEach(constituency -> System.out.print(" "  + constituency + "=" + constituency.getCandidates(party).size()));
    ...
    public String prepareSummary() {
        ...
        System.out.println(this.getParties().size() + " Participating parties:");
        // TODO report the list of all parties ordered by increasing party-Id
        System.out.println(this.getParties());
        // TODO report the total number of constituencies in the election
        System.out.println("Total number of constituencies = " + this.getConstituencies().size());
        // TODO report the total number of polling stations in the election
        AtomicInteger numberOfPollingstations = new AtomicInteger();
        this.constituencies.forEach(constituency -> {
            numberOfPollingstations.addAndGet(constituency.getPollingStations().size());
        });
        System.out.println("Total number of polling stations = " + numberOfPollingstations);
        // TODO report the total number of (different) candidates in the election
        System.out.println("Total number of candidates in the election = " + this.getAllCandidates().size());
        // TODO report the list with all candidates which have a counter part with a duplicate name in a different party
        System.out.println("Different candidates with duplicate names across different parties are:");
        System.out.println(this.getCandidatesWithDuplicateNames());
        System.out.println();
        // TODO report the sorted list of overall election results ordered by decreasing party percentage
        System.out.println("Overall election results by party percentage:");
        System.out.println(sortedElectionResultsByPartyPercentage(getParties().size(), getVotesByParty()));
        System.out.println();
        // TODO report the polling stations within the Amsterdam Wibautstraat area with zipcodes between 1091AA and 1091ZZ
        System.out.println("Polling stations in Amsterdam Wibautstraat area with zip codes 1091AA-1091ZZ:");
        System.out.println(this.getPollingStationsByZipCodeRange("1091AA", "1091ZZ"));
        System.out.println();
        ...
        System.out.println("Top 10 election results by party percentage in Amsterdam area with zip codes 1091AA-1091ZZ:");
        System.out.println(sortedElectionResultsByPartyPercentage(10, this.getVotesByPartyAcrossPollingStations(this.getPollingStationsByZipCodeRange("1091AA", "1091ZZ"))));
        System.out.println();
        // TODO report the most representative polling station across the election
        System.out.println("Most representative polling station is:");
        System.out.println(this.findMostRepresentativePollingStation());
        System.out.println();
        // TODO report the sorted election results by decreasing party percentage of the most representative polling station
        System.out.println(sortedElectionResultsByPartyPercentage(this.findMostRepresentativePollingStation().getVotesByParty().size(), this.findMostRepresentativePollingStation().getVotesByParty()));
