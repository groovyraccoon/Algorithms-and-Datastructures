...
public class Constituency {
    ...
    public boolean register(int rank, Candidate candidate) {
        ...
        if (!rankedCandidatesByParty.containsKey(candidate.getParty())) {
            Map<Integer, Candidate> map = new TreeMap<>();
            rankedCandidatesByParty.put(candidate.getParty(), (NavigableMap<Integer, Candidate>) map);
            map.put(rank, candidate);

            return true;
        }

        if (rankedCandidatesByParty.get(candidate.getParty()).containsKey(rank) || rankedCandidatesByParty.get(candidate.getParty()).containsValue(candidate)) return false;

        rankedCandidatesByParty.get(candidate.getParty()).put(rank, candidate);
    ...
    public final List<Candidate> getCandidates(Party party) {
        ...
        return new ArrayList<Candidate>(rankedCandidatesByParty.get(party).values()); // replace by a proper outcome
    }
    ...
    public Set<Candidate> getAllCandidates() {
        ...
        return rankedCandidatesByParty.values().stream().flatMap(x -> x.values().stream()).collect(Collectors.toSet());   // replace by a proper outcome
    }
    ...
    public NavigableSet<PollingStation> getPollingStationsByZipCodeRange(String firstZipCode, String lastZipCode) {
        ...
        return pollingStations.subSet(new PollingStation(null, firstZipCode, null), true, new PollingStation(null, lastZipCode, null), true);
    }
    ...
    public Map<Party, Integer> getVotesByParty() {
        ...
        Map<Party, Integer> totalPartyVote = new HashMap<>();
        Map<Candidate, Integer> votesByCandidate = new HashMap<>();


        pollingStations.forEach(p -> {
            p.getVotesByCandidate().forEach( (candidate, integer) -> {
                votesByCandidate.put(candidate, votesByCandidate.getOrDefault(candidate, 0) + integer);
            });
        });

        votesByCandidate.forEach((candidate, integer) -> {
            totalPartyVote.put(candidate.getParty(), totalPartyVote.getOrDefault(candidate.getParty(), 0) + integer);
        });

        return  totalPartyVote; // replace by a proper outcome
    }
