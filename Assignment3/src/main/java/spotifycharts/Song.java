...
public class Song{
    ...
    private Map<String, Integer> StreamsPerCountry;
...
//        this.StreamsPerCountry.put(country, streams);
    }
    ...
    public void setStreamsCountOfCountry(Country country, int streamsCount) {
        ...
        this.StreamsPerCountry.put(country.name(), streamsCount);
    }
    ...
    public int getStreamsCountOfCountry(Country country) {
        ...
        if (this.StreamsPerCountry.get(country.name()) == null){
            return 0;
        }
        return this.StreamsPerCountry.get(country.name()); // replace by the proper amount
    }
    ...
    public int getStreamsCountTotal() {
        ...
        int totalStreamCount =0;
            for (Map.Entry<String, Integer> entry : this.StreamsPerCountry.entrySet()){
               totalStreamCount += entry.getValue();
            }

        return totalStreamCount; // replace by the proper amount
    }
    ...
    public int compareByHighestStreamsCountTotal(Song other) {
        ...
        return  Integer.compare(other.getStreamsCountTotal(), getStreamsCountTotal());    // replace by proper result
    }
    ...
    public String toString() {
        ...
        return String.format("%s/%s{%s}(%s)", this.artist, this.title, this.language, this.getStreamsCountTotal());
    }
}
