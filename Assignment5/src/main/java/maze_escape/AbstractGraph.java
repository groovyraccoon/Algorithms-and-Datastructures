...
public abstract class AbstractGraph<V> {
    ...
    public Set<V> getAllVertices(V firstVertex) {
        ...
        //method below
        return recursiveGetAll(vSet, firstVertex);    // replace by a proper outcome
    }

    public Set<V> recursiveGetAll(Set<V> vSet, V vertex) {

        Set<V> neighbour = new HashSet<>();
        neighbour = getNeighbours(vertex);
        vSet.add(vertex);

        for (V v : neighbour) {
            if (!vSet.contains(v)) {
                vSet.addAll(recursiveGetAll(vSet, v));
            }
        }
        return vSet;
    ...
    public String formatAdjacencyList(V firstVertex) {
        ...
        Set<V> vSet = getAllVertices(firstVertex);

        String neighbours = getNeighbours(firstVertex).toString().replace(" ", "");
        stringBuilder.append(firstVertex + ": " + neighbours + "\n");

        for (V v : vSet) {
            if (v != firstVertex) {
                neighbours = getNeighbours(v).toString().replace(" ", "");
                stringBuilder.append(v + ": " + neighbours + "\n");
            }
        }
    ...
    public GPath depthFirstSearch(V startVertex, V targetVertex) {
        ...
        boolean pathFound = depthFirstSearchRecursive(startVertex, targetVertex, path);

        return pathFound ? path : null;
    }

    public boolean depthFirstSearchRecursive(V start, V target, GPath path) {
        if (path == null) return false;

        if (path.visited.contains(start)) return false;
        ...
        for (V neighbor : this.getNeighbours(start)) {
            pathFound = depthFirstSearchRecursive(neighbor, target, path);
    ...
    public GPath breadthFirstSearch(V startVertex, V targetVertex) {
        ...
        // If the startVertex is equal to the targetVertex it gets added
        ...
        while (active != null) {
            for (V neighbor : this.getNeighbours(active)) {
                path.visited.add(neighbor);

                if (neighbor.equals(targetVertex)) {
                    while (active != null) {
                        path.vertices.addFirst(active);
                        active = visitedFrom.get(active);
                    }

                    return path;
                } else if (!visitedFrom.containsKey(neighbor)) {
                    visitedFrom.put(neighbor, active);
    ...
    public GPath dijkstraShortestPath(V startVertex, V targetVertex,
        ...
        while (!priorityQueue.isEmpty()) {
            ...
            if (nearestMSTNode.marked) continue;

            nearestMSTNode.marked = true;

            if (nearestMSTNode.vertex.equals(targetVertex)) {
                return getShortestPath(startVertex, path, minimumSpanningTree, nearestMSTNode);
            }

            for (V neighbor : getNeighbours(nearestMSTNode.vertex)) {
                updateWeightMSTNode(weightMapper, path, minimumSpanningTree, nearestMSTNode, priorityQueue, neighbor);
            }
        }
    ...
    private GPath getShortestPath(V startVertex, GPath path, Map<V, MSTNode> minimumSpanningTree, MSTNode nearestMSTNode) {
        path.totalWeight = nearestMSTNode.weightSumTo;
        Stack<V> stack = new Stack<>();
        while (nearestMSTNode.parentVertex != null) {
            stack.push(nearestMSTNode.vertex);
            nearestMSTNode = minimumSpanningTree.get(nearestMSTNode.parentVertex);
        }

        stack.push(startVertex);
        while (!stack.isEmpty()) {
            path.vertices.add(stack.pop());
        }

        return path;
    }

    private void updateWeightMSTNode(BiFunction<V, V, Double> weightMapper, GPath path, Map<V, MSTNode> minimumSpanningTree,
                                     MSTNode nearestMSTNode, PriorityQueue<MSTNode> priorityQueue, V neighbor) {
        double weight = weightMapper.apply(nearestMSTNode.vertex, neighbor);
        ...
        if (!neighborNode.marked) {
            double newWeight = nearestMSTNode.weightSumTo + weight;
